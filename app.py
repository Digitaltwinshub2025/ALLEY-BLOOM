...
def add_cache_headers(response):
    # Disable caching for static files and templates
    if 'static' in request.path or request.path.endswith('.html'):
        response.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate, max-age=0'
        response.headers['Pragma'] = 'no-cache'
        response.headers['Expires'] = '0'
        response.headers['ETag'] = None
        response.headers['Last-Modified'] = datetime.utcnow().strftime('%a, %d %b %Y %H:%M:%S GMT')
    # Also disable cache for all template routes
    if request.path == '/' or '/studio' in request.path or '/workspace' in request.path or '/viewer' in request.path or '/dashboard' in request.path or '/library' in request.path or '/map' in request.path or '/scenarios' in request.path or '/before-after' in request.path:
        response.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate, max-age=0'
        response.headers['Pragma'] = 'no-cache'
        response.headers['Expires'] = '0'
    return response
...
@app.route('/existing')
def existing():
    # Existing conditions page with Street View and NASA data
    google_api_key = GOOGLE_API_KEY if GOOGLE_API_KEY else ''
    edit_mode = request.args.get('edit') == 'true' and is_edit_allowed()
    page_content = content_manager.load_page_content('existing') or {}
    return render_template('existing.html', google_api_key=google_api_key, edit_mode=edit_mode, content=page_content)
...
@app.route('/fence-map')
def fence_map():
    # Interactive fence map - spatial navigation of fence segments linked to cost data
